...
exports.run = async (bot, message, args) => {
  ...
  if (song === null) songloop.set(`song_${message.guild.id}`, false);
  if (queue === null) queueloop.set(`song_${message.guild.id}`, false);
  ...
  if (response.first().content.toLowerCase() === 'song') {
    ...
    if (song === false && queue === false) return message.channel.send('Nothing is currently being looped.')
  }
}
