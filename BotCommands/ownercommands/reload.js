...
exports.run = async (bot, message, args) => {
  ...
  let imagecommands = ['brazzers', 'bunny', 'cat', 'deepfry', 'dog', 'duck', 'explode', 'funky', 'implode', 'invert', 'magik', 'meme', 'owl', 'penguin', 'photo', 'rainbow', 'retarded'];
  ...
  try {
    ...
    musiccommands.forEach(musiccommand => {
      if (args[0] == musiccommand) {
        delete require.cache[require.resolve(`../musiccommand/${args[0]}.js`)];
        console.log(`${args[0]}.js successfully reloaded!`);
        return message.channel.send(Aembed);
      }
    });
    ...
    songcommands.forEach(songcommand => {
      if (args[0] == songcommand) {
        delete require.cache[require.resolve(`../songcommand/${args[0]}.js`)];
        console.log(`${args[0]}.js successfully reloaded!`);
        return message.channel.send(Aembed);
      }
    });
