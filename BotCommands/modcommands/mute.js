...
exports.run = async(bot, message, args) => {
  ...
  .setColor('PURPLE')
  .addField('Command usage', `${prefix}mute <member name / mention> <reason optional>`)
  .addField('Server mute usage', `${prefix}mute server <member name / mention> <reason optional>`)

  if (!Mute) return message.reply(errorEmbed);
  let arg = message.content.toLowerCase(); 
  let mreason = args.join(" ").slice(22);

  if (Mute.hasPermission("MANAGE_MESSAGES") || Mute.roles.has(mr)) return message.reply("Can't mute them!"); 
