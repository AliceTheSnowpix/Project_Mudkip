...
exports.run = async(bot, message, args) => {
  ...
  if (cmd === 'welcomechannel') {
    ...
    const wc = welcomechannel.fetch(`welcomechannel_${message.guild.id}`);
    if (!wc) {
  ...
  if (cmd === 'leavechannel') {
    ...
    const lc = leavechannel.fetch(`leavechannel_${message.guild.id}`);
    if (!lc) leavechannel.set(`leavechannel_${message.guild.id}`, channel.id);
    else leavechannel.set(`leavechannel_${message.guild.id}`, channel.id);
  ...
  if (cmd === 'autorole') {
    ...
    const ar = autorole.fetch(`autorole_${message.guild.id}`)
    if (!ar) autorole.set(`autorole_${message.guild.id}`, gRole.id);
    else autorole.set(`autorole_${message.guild.id}`, gRole.id);
  ...
  if (cmd === 'list') {
    ...
    if (wc === null) welcomechannel.set(`welcomechannel_${message.guild.id}`, 'not set');
    const lc = await leavechannel.fetch(`leavechannel_${message.guild.id}`);
    if (lc === null) leavechannel.set(`leavechannel_${message.guild.id}`, 'not set');
    const mc = await logchannel.fetch(`logchannel_${message.guild.id}`);
    if (mc === null) logchannel.set(`logchannel_${message.guild.id}`, 'not set');
    const wm = await welcomemessage.fetch(`welcomemessage_${message.guild.id}`);
    if (wm === null) welcomemessage.set(`welcomemessage_${message.guild.id}`, 'not set');
    const lm = await leavemessage.fetch(`leavemessage_${message.guild.id}`);
    if (lm === null) leavemessage.set(`leavemessage_${message.guild.id}`, 'not set');
    const ar = await autorole.fetch(`autorole_${message.guild.id}`);
    if (ar === null) autorole.set(`autorole_${message.guild.id}`, 'not set');
    const mr = await modrole.fetch(`modrole_${message.guild.id}`);
    if (mr === null) modrole.set(`modrole_${message.guild.id}`, 'not set');
