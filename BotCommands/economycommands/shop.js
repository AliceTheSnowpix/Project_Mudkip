...
exports.run = async(bot, message, args) => {
  ...
  if (message.guild.id != serverID) {
    ...
    let cmd = message.content.slice(prefix.length).toLowerCase().split(' ').slice(1, 2).join(' ').toLowerCase();
    if (!cmd) {
      ...
      if (n1 === 'not set' && c1 === 'not set' && i1 === 'not set' 
          ...
          && n10 === 'not set' && c10 === 'not set' && i10 === 'not set') return message.channel.send('There is currently no items on the shop');
      ...
      if (n1 === null && c1 === null && i1 === null 
          ...
          && n10 === null && c10 === null && i10 === null) return message.channel.send('There is currently no items on the shop');
...
      else shopEmbed.addField(`${n10}`, `${i10}, you need to pop and pay ${c10} bubbles to buy this item.`);
    ...
    Name1.set(`name1_${serverID}`, '1');
    Name2.set(`name2_${serverID}`, '2');
    Name3.set(`name3_${serverID}`, '3');
    Name4.set(`name4_${serverID}`, '4');
    Name5.set(`name5_${serverID}`, '5');
    Name6.set(`name6_${serverID}`, '6');
    Name7.set(`name7_${serverID}`, '7');
    Name8.set(`name8_${serverID}`, '8');
    Name9.set(`name9_${serverID}`, '9');
    Name10.set(`name10_${serverID}`, '10');
